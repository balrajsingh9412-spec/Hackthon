...
async function runSecurityAudit() {
  ...
  try {
    ...
    // User A currently has 100 starting Gold + 60 (epic quest) = 160 Gold.
    // Elixir of Wisdom costs 150 Gold. Send 5 simultaneous purchase requests!
    const concurrentBuys = await Promise.all(
      Array.from({ length: 5 }).map(() => req('/shop/elixir_wisdom/buy', { method: 'POST', token: tokenA }))
    );
    ...
    console.log('\n🔒 ALL SECURITY AUDIT TESTS PASSED PERFECTLY!');
    return results;
  } catch (err) {
